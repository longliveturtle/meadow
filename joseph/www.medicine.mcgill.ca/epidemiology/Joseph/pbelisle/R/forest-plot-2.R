                           ...
                           )                          
{
  ...
  or.bayesianci <- function(m1,n1,m0,n0, gr1.over.gr0=T, level=.95, M=100000, J=0)
  {
    ...
    ci
  } # end of or.bayesianci
  
  
  plot.ticks <- function(plot.lim, tick.lim, add.ticks.at, ref.vline.at, plot.xaxis, y.xaxis, log.scale, a, b, report.meta.ci, tick.pch, yunit, cex)
  {
    x.ticks <- add.ticks.at
    x.ticks <- x.ticks[x.ticks >= min(plot.lim) & x.ticks <= max(plot.lim)]
    
    if (tick.lim) x.ticks <- unique(c(x.ticks, plot.lim))
    x.text <- x.ticks
    x.lim <- plot.lim
    ...
    text.yloc <- as.numeric(!report.meta.ci) * yunit
    text.xloc <- a + b*x.ticks
    text(text.xloc, text.yloc, x.text, adj=0.5, cex=cex)
    
    points(a+b*x.ticks, rep(yunit, length(x.ticks)), pch=tick.pch, adj=.5) 
    
    # x-axis
    if (plot.xaxis) points(a+b*x.lim, rep(y.xaxis,2), type='l')
  } # end of plot.ticks
  ...
  Round <- function(x, len, min.ndecimals)
  {
    ...
    {
      w <- x >= 0 
      x.int <- numeric(length(x))
      if (any(w))  x.int[w]  <- floor(x[w])    # for positive numbers
      if (any(!w)) x.int[!w] <- ceiling(x[!w]) # for negative numbers
      
      x.fraction <- abs(x - x.int)
      ...
      # seq(along=ndigits2complete.with)
      for (i in which(ndigits2complete.with>0))
      {
      ...
      lx <- nchar(x)
      for (i in which(lx<len))
      {
        dot.already.in <- regexpr("\\.", x[i]) > 0
        nzeros2add <- len - lx[i] - !dot.already.in
        
        x[i] <- paste(c(x[i], ifelse(dot.already.in, "", "."), rep(0, nzeros2add)), collapse="")
      }
  ...
  options(scipen=or.min.ndecimals+1)
  
  if (length(m0) != length(n0))
  {
    msg <- "m0 and n0 must be of same length."
    stop(msg)
  }
  
  if (length(m1) != length(n1))
  {
    msg <- "m1 and n1 must be of same length."
    stop(msg)
  }
  
  if (length(m0) != length(authors) && length(m0) > 0)
  {
    msg <- "m0 and n0 must be of same length as authors."
    stop(msg)
  }
  ...
  if (length(study.categories) > 0 && length(study.categories) != length(authors))
  {
    msg <- "authors and study.categories must be of same length. Sorry."
    stop(msg)
  } 
  
  if (length(study.categories) > 0)
  {
    if (length(category.labels) == 0)
    {
      msg <- "category.labels was left undefined while study.categories is used."
      stop(msg)
    }
    
    tmp <- max(study.categories)
    if (tmp > length(category.labels))
    {
      msg <- "Maximum index in study.categories is greater than the number of categories defined in category.labels"
      stop(msg)
    }
  }
  
  if (nrow(categories.ci) > 0 && nrow(categories.ci) != length(study.categories))
  {
    msg <- "The number of rows in categories.ci must match the length of study.categories"
    stop(msg)
  }
  
  if (nrow(studies.ci) > 0 &&  nrow(studies.ci) != length(authors))
  {
    msg <- "The number of rows in studies.ci must match the number of studies, that is, the length of authors."
    stop(msg)
  }
  
  if (length(plot.categories.ci) > 0 && length(plot.categories.ci) != length(category.labels))
  {
    msg <- "plot.categories.ci and category.labels must be of same length."
    stop(msg)
  }
  
  if (is.matrix(meta.ci))
  {
    if (nrow(meta.ci) > 1 || ncol(meta.ci) != 3)
    {
      msg <- "meta.ci must be a vector of length 3."
      stop(msg)
    }
    else
    {
      meta.ci <- as.vector(meta.ci)
    }
  }
  ...
  # Make sure categories.ci is defined if any of plot.categories.ci is True
  
  if (any(plot.categories.ci))
  {
    tmp <- length(plot.categories.ci)
    tmp2 <- nrow(categories.ci)
    
    if (tmp2 != tmp) stop("categories.ci must have as many rows as plot.categories.ci has elements.")
    
    plot.categories.ci <- plot.categories.ci & apply(!is.na(categories.ci),1,all)
    
    w <- which(plot.categories.ci)
    tmp <- categories.ci[w,]
    if (any(is.na(tmp))) stop("categories.ci must be defined / not present NAs for categories for which CIs are to be plotted (as indicated by argument plot.categories.ci).")  
  }
  ...
  rightScale.plot.lim <- plot.lim
  if (log.scale) rightScale.plot.lim <- log(rightScale.plot.lim)

  x1 <- rightScale.plot.lim[1]
  x2 <- rightScale.plot.lim[2]
  z1 <- plotlim0
  z2 <- plotlim1
  ...
  if (test)
  {
    or.ci <- matrix(rightScale.plot.lim*c(.9,1.1), ncol=2, nrow=nstudies, byrow=T)
  ...
  {
    nN.txt <- paste('\n', nN.txt, sep='')
    group.labels <- paste(group.labels, nN.txt, sep='')
  }
  ...
  {
    x <- ref.vline.at
    if (log.scale) x <- log(x)
    x <- a + b*x
    points(rep(x,2), y, type='l')
  }
  
  plot.ticks(plot.lim, tick.lim, add.ticks.at, ref.vline.at, plot.xaxis, y.xaxis, log.scale, a, b, report.meta.ci, tick.pch, yunit, cex)  
